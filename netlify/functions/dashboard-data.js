...
exports.handler=async(event)=>{
  ...
  try{
    ...
    if(newRefreshToken&&newRefreshToken!==XERO_REFRESH_TOKEN){try{await fetch('https://api.netlify.com/api/v1/accounts/'+process.env.NETLIFY_ACCOUNT_ID+'/env/XERO_REFRESH_TOKEN',{method:'PATCH',headers:{'Authorization':'Bearer '+process.env.NETLIFY_API_TOKEN,'Content-Type':'application/json'},body:JSON.stringify({values:[{context:'production',value:newRefreshToken}]})});}catch(e){console.warn('Token rotation failed:',e.message);}}
